...
exp5 = load_module("exp5_base_for_exp8", CODE_DIR / "exp5_gpu_corrected" / "run.py")
base = exp5.base
...
def preprocessing_cache_path(data_path: Path, cfg: dict, model_dir: Path) -> Path:
    ...
    generic = cache_dir / f"graph_preprocessed_{digest}.pt"
    exp7_cache = cache_dir / f"exp7_preprocessed_{digest}.pt"
    return exp7_cache if exp7_cache.exists() and not generic.exists() else generic


def graph_cache_payload(graph, feature_names):
    """Serialize only compact tensors needed by EXP8, not Python adjacency lists."""
    return {
...
class AdaptiveBatchedExactGPUSampler(exp5.ExactGPUSampler):
    """Exact importance sampler with adaptive GPU batches and reusable weights.

    The EXP8 graph is explicitly bidirectional. The final PPR step can therefore
    gather through reverse adjacency instead of expanding the complete last-hop
    state. Root/state groups are packed up to an edge budget, while a hub that
    exceeds the budget is streamed in slices. These execution choices preserve
    the local-PPR values and weighted Gumbel-top-k distribution.
    """

    def __init__(self, edge_index, num_nodes, known_labels, strategy, fanouts,
                 cfg, device, seed, shared_importance_cache=None):
        super().__init__(
        ...
        self.importance_max_roots_per_batch = max(
            1, int(cfg.get("importance_max_roots_per_batch", 128))
        )
        self.cache_importance_weights = bool(
            cfg.get("cache_importance_weights", True)
        )
        if self.ppr_steps < 1:
            raise ValueError("sampling.ppr_steps harus minimal 1.")
        self.importance_weight_cache = None
        self.importance_cached_roots = None
        self.importance_cache_stats = {"root_hits": 0, "root_misses": 0}
        if strategy == "importance" and self.cache_importance_weights:
            shared_importance_cache = (
                shared_importance_cache
                if shared_importance_cache is not None else {}
            )
            if shared_importance_cache:
                self.importance_weight_cache = shared_importance_cache["weights"]
                self.importance_cached_roots = shared_importance_cache["cached_roots"]
                if self.importance_weight_cache.numel() != self.col.numel():
                    raise ValueError("Cache bobot importance tidak cocok dengan graph.")
            else:
                self.importance_weight_cache = torch.empty(
                    self.col.numel(), dtype=torch.float32, device=self.device
                )
                self.importance_cached_roots = torch.zeros(
                    self.num_nodes, dtype=torch.bool, device=self.device
                )
                shared_importance_cache.update({
                    "weights": self.importance_weight_cache,
                    "cached_roots": self.importance_cached_roots,
                })

    def _edge_bounded_slices(self, nodes, max_items=None):
        """Plan slices after one device-to-host transfer of their degree vector."""
        if nodes.numel() == 0:
            return
        degrees = self.degree_long[nodes].detach().cpu().numpy()
        cumulative = np.cumsum(degrees, dtype=np.int64)
        first = 0
        while first < len(degrees):
            before = 0 if first == 0 else int(cumulative[first - 1])
            edge_last = int(np.searchsorted(
                cumulative, before + self.importance_edge_chunk_size, side="right"
            ))
            if max_items is None:
                item_last = len(degrees)
            else:
                item_last = min(len(degrees), first + int(max_items))
            last = max(first + 1, min(edge_last, item_last))
            yield first, last, int(degrees[first]) > self.importance_edge_chunk_size
            first = last

    def _merge_sparse(self, keys, masses, new_keys, new_masses):
        """Coalesce a contribution chunk into a sorted composite-key state."""
        if new_keys.numel() == 0:
            return keys, masses
    ...
    def _expand_large_state(self, state_root, node, mass, keys, masses):
        degree = int(self.degree_long[node].item())
        if degree == 0:
            return keys, masses
        row_start = int(self.rowptr[node].item())
        share = (1 - self.beta) * mass / degree
        root_key_offset = state_root * self.num_nodes
        for offset in range(0, degree, self.importance_edge_chunk_size):
            ...
            new_keys = root_key_offset + destinations
            keys, masses = self._merge_sparse(
                keys, masses, new_keys, share.expand(length)
            )
        return keys, masses

    def _bounded_ppr_step(self, roots, state_roots, state_nodes, state_mass):
        root_ids = torch.arange(roots.numel(), device=self.device)
        keys = root_ids * self.num_nodes + roots
        masses = torch.full(
            (roots.numel(),), self.beta, dtype=state_mass.dtype, device=self.device
        )
        for first, last, oversized in self._edge_bounded_slices(state_nodes):
            group_nodes = state_nodes[first:last]
            group_roots = state_roots[first:last]
            group_mass = state_mass[first:last]
            group_degrees = self.degree_long[group_nodes]
            if oversized:
                keys, masses = self._expand_large_state(
                    group_roots[0], group_nodes[0], group_mass[0], keys, masses
                )
                continue
            segments, positions = self._ragged_edges(group_nodes)
            if positions.numel() == 0:
                continue
            expanded_roots = torch.repeat_interleave(group_roots, group_degrees)
            new_keys = expanded_roots * self.num_nodes + self.col[positions]
            shares = (
                (1 - self.beta) * group_mass / group_degrees.clamp_min(1).float()
            )
            contributions = torch.repeat_interleave(shares, group_degrees)
            keys, masses = self._merge_sparse(
                keys, masses, new_keys, contributions
            )
        return (
            torch.div(keys, self.num_nodes, rounding_mode="floor"),
            keys.remainder(self.num_nodes),
            masses,
        )

    def _mass_at_keys(self, state_keys, state_mass, lookup_keys):
        indices = torch.searchsorted(state_keys, lookup_keys)
        safe_indices = indices.clamp_max(state_keys.numel() - 1)
        found = (indices < state_keys.numel()) & (
            state_keys[safe_indices] == lookup_keys
        )
        return torch.where(
            found, state_mass[safe_indices], torch.zeros_like(lookup_keys).float()
        )

    def _project_last_ppr_step(self, roots, state_roots, state_nodes, state_mass,
                               candidate_segments, candidates):
        state_keys = state_roots * self.num_nodes + state_nodes
        scores = torch.where(
            candidates == roots[candidate_segments],
            torch.full(candidates.shape, self.beta, device=self.device),
            torch.zeros(candidates.shape, device=self.device),
        )
        for first, last, oversized in self._edge_bounded_slices(candidates):
            group = candidates[first:last]
            group_root_ids = candidate_segments[first:last]
            if oversized:
                node = group[0]
                degree = int(self.degree_long[node].item())
                row_start = int(self.rowptr[node].item())
                contribution_sum = torch.zeros((), device=self.device)
                root_key_offset = group_root_ids[0] * self.num_nodes
                for offset in range(0, degree, self.importance_edge_chunk_size):
                    length = min(self.importance_edge_chunk_size, degree - offset)
                    sources = self.col[
                        row_start + offset:row_start + offset + length
                    ]
                    incoming_mass = self._mass_at_keys(
                        state_keys, state_mass, root_key_offset + sources
                    )
            ...
            lookup_keys = (
                group_root_ids[segments] * self.num_nodes + sources
            )
            incoming_mass = self._mass_at_keys(state_keys, state_mass, lookup_keys)
            contributions = (
    ...
    def _propagate_local_ppr(self, roots, candidate_segments, candidate_nodes):
        """Compute exact independent PPR walks for an adaptive batch of roots."""
        state_roots = torch.arange(roots.numel(), device=self.device)
        state_nodes = roots
        state_mass = torch.ones(roots.numel(), device=self.device)
        for _ in range(self.ppr_steps - 1):
            state_roots, state_nodes, state_mass = self._bounded_ppr_step(
                roots, state_roots, state_nodes, state_mass
            )
        return self._project_last_ppr_step(
            roots, state_roots, state_nodes, state_mass,
            candidate_segments, candidate_nodes,
        )

    def _compute_importance_weights(self, roots, segments, positions):
        candidates = self.col[positions]
        ppr = self._propagate_local_ppr(roots, segments, candidates)
    ...
    def _importance_weights(self, roots, segments, positions):
        if self.importance_weight_cache is None:
            return self._compute_importance_weights(roots, segments, positions)

        cached = self.importance_cached_roots[roots]
        uncached = ~cached
        if bool(uncached.any().item()):
            edge_mask = uncached[segments]
            root_remap = torch.cumsum(uncached.long(), dim=0) - 1
            uncached_roots = roots[uncached]
            uncached_segments = root_remap[segments[edge_mask]]
            uncached_positions = positions[edge_mask]
            computed = self._compute_importance_weights(
                uncached_roots, uncached_segments, uncached_positions
            )
            self.importance_weight_cache[uncached_positions] = computed
            self.importance_cached_roots[uncached_roots] = True
            self.importance_cache_stats["root_misses"] += int(uncached_roots.numel())
        self.importance_cache_stats["root_hits"] += int(cached.sum().item())
        return self.importance_weight_cache[positions]
    ...
    def _sample_large_nodes(self, roots, fanout):
        ...
        groups = self._edge_bounded_slices(
            roots, max_items=self.importance_max_roots_per_batch
        )
...
def resolve_device(requested: str | None) -> torch.device:
    """Require CUDA so an EXP8 run can never silently train on the CPU."""
    ...
    if device.type != "cuda":
        raise ValueError(
            "EXP8 bersifat GPU-bound; set experiment.device: cuda."
        )
    if not torch.cuda.is_available():
        raise RuntimeError(
            "EXP8 memerlukan PyTorch CUDA dan GPU NVIDIA yang tersedia.\n"
...
def run_one(cfg, graph, strategy, seed, model_dir, result_dir, device, data_stats,
    ...
    train_sampler = AdaptiveBatchedExactGPUSampler(
    ...
    if amp_requested and not amp_enabled:
        LOGGER.warning("AMP dinonaktifkan karena EXP8 berjalan pada CPU.")
    ...
    checkpoint = model_dir / f"exp8_{strategy}_seed{seed}.pt"
    torch.save({
        ...
        "sampler": "exact_torch_gumbel_topk_multigraph_adaptive_cached",
    }, checkpoint)
    metrics.update({
        "experiment": "exp8_adaptive_importance",
        "strategy": strategy,
        "seed": seed,
        "best_val_auprc": best,
    ...
    output = result_dir / f"exp8_{strategy}_seed{seed}.json"
    output.write_text(json.dumps({
        ...
        "sampling": {
            ...
            "importance_max_roots_per_batch": (
                train_sampler.importance_max_roots_per_batch
            ),
            "cache_importance_weights": train_sampler.cache_importance_weights,
            "importance_cache_root_hits": (
                train_sampler.importance_cache_stats["root_hits"]
            ),
            "importance_cache_root_misses": (
                train_sampler.importance_cache_stats["root_misses"]
            ),
            "importance_cache_memory_gb": (
                0.0 if train_sampler.importance_weight_cache is None else (
                    train_sampler.importance_weight_cache.numel()
                    * train_sampler.importance_weight_cache.element_size()
                    + train_sampler.importance_cached_roots.numel()
                    * train_sampler.importance_cached_roots.element_size()
                ) / 1024**3
            ),
...
def main():
    ...
    try:
        ...
        if cache_loaded:
            ...
            overall.set_description("EXP8 | membaca dataset")
        ...
        # EXP8 uses the sampler CSR for long-tail audit; these duplicate graph
        # representations are not needed during training.
        graph.data = None
        graph.adjacency = None
        gc.collect()

        overall.set_description("EXP8 | audit split")
        overall.set_postfix_str(f"{graph.x.shape[0]:,} node")
        data_stats = collect_split_stats(graph)
        overall.update(1)

        if args.preprocess_only:
            overall.set_description("EXP8 | preprocessing siap")
            overall.set_postfix_str(cache_path.name)
            overall.update(1)
        ...
        for run_index, (strategy, seed) in enumerate(combinations, start=1):
            overall.set_description(f"EXP8 | run {run_index}/{len(combinations)}")
        ...
        overall.set_description("EXP8 | menyimpan ringkasan")
        overall.set_postfix_str("CSV + hasil akhir")
        summary = result_dir / "exp8_summary.csv"
        ...
        overall.update(1)
        overall.set_description("EXP8 | selesai")
