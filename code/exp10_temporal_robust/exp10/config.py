...
def validate_config(cfg):
    if cfg['features']['encoder'] not in {'robust', 'legacy'} or cfg['features']['storage_dtype'] not in {'float16', 'float32'}:
        raise ValueError('features.encoder robust/legacy; storage_dtype float16/float32')
    ...
    if tc['selection_metric'] not in {'ap', 'temporal_geometric_ap_lift'}:
    ...
    if mc["num_layers"] != 2 or len(sc["fanouts"]) != 2 or any(not isinstance(k, int) or k < 2 for k in sc["fanouts"]):
        raise ValueError("EXP10 khusus dua lapisan dengan setiap fanout >= 2")
    if sc["ppr_steps"] != 3:
        raise ValueError("Closed-form PPR EXP10 memerlukan ppr_steps=3")
    ...
    for section, names in {
        ...
        "training": ["batch_size", "epochs", "early_stopping_patience"],
