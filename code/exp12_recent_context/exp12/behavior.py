...
def card_codes(df, user, policy='legacy'):
    available = 'Card' in df and df['Card'].notna().all()
    if policy == 'per_row' and 'Card' in df:
        # Missing cards form one unknown-card history PER USER. One missing
        # value must not collapse every known card into its user's history.
        return _pair(user, _codes(df['Card'].fillna('<missing>').astype(str))), bool(available)
    return (_pair(user, _codes(df['Card'])) if available else user), bool(available)


def fill_behavior_features(df, output, card_policy='legacy'):
    ...
    user = _codes(df['User'])
    card, card_available = card_codes(df, user, card_policy)
    if not card_available and card_policy == 'legacy':
        logging.getLogger('exp12').warning('Card kosong/tidak tersedia: histori card memakai histori user')
