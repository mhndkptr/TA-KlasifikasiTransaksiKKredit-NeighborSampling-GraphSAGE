...
from .behavior import FEATURE_NAMES as BEHAVIOR_NAMES, fill_behavior_features
from .behavior_v2 import FEATURE_NAMES as CONTEXT_NAMES, fill_context_features
...
class TransactionGraph:
    ...
    num_users: int
    metadata: dict
    payment_channels: torch.Tensor | None = None
...
def build_graph(df, ratios, feature_config=None):
    ...
    behavioral = feature_config['encoder'] == 'behavioral'
    encoder_class = LegacyFeatureEncoder if feature_config['encoder'] == 'legacy' else FeatureEncoder
    encoder = encoder_class().fit(df.iloc[:train_end])
    contextual = feature_config['encoder'] == 'contextual'
    names = encoder.feature_names + (CONTEXT_NAMES if contextual else BEHAVIOR_NAMES if behavioral else [])
    features = np.empty((n, len(names)), dtype=feature_config['storage_dtype'])
    metadata = {"feature_names": names, "encoder": encoder.state,
    ...
    for name, lo, hi in (("train", 0, train_end), ("val", train_end, val_end), ("test", val_end, n)):
        ...
        for start in range(lo, hi, 250000):
            ...
            features[start:end, :len(encoder.feature_names)] = values
    ...
    if behavioral:
        metadata['behavior'] = fill_behavior_features(df, features[:, len(encoder.feature_names):])
    elif contextual:
        metadata['behavior'] = fill_context_features(df, features[:, len(encoder.feature_names):])
    # Channel IDs are reporting/training-stratum metadata, never ordinal input.
    channels = list(pd.unique(df['Use Chip'].fillna('<missing>').astype(str)))
    metadata['payment_channel_names'] = channels
    channel_codes = df['Use Chip'].fillna('<missing>').astype(str).map(
        {name: i for i, name in enumerate(channels)}).to_numpy(np.int8)
    ...
    return TransactionGraph(
        ...
        train_end, val_end, num_users, metadata, torch.from_numpy(channel_codes),
    )
