...
class GraphSAGE(nn.Module):
    def __init__(self, input_channels, hidden=256, dropout=0.2, normalization='batch', use_graph_context=True):
        super().__init__()
        ...
        self.norms = nn.ModuleList([norm(hidden), norm(hidden)])
    ...
    def forward(self, features, blocks):
        for i, block in enumerate(blocks):
            if self.use_graph_context:
                ...
                features = self.from_mean(i, features[block.self_index], torch.zeros_like(features[block.self_index]))
    ...
    def from_mean(self, layer, self_features, mean_neighbors):
        ...
        if not self.use_graph_context:
            mean_neighbors = torch.zeros_like(mean_neighbors)
