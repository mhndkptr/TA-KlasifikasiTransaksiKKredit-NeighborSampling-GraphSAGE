...
from .data import FeatureEncoder, load_transactions, split_boundaries
...
def build_graph(df, ratios):
    n = len(df)
    train_end, val_end = split_boundaries(n, ratios)
    labels = df["_label"].to_numpy(np.int8)
    encoder = FeatureEncoder().fit(df.iloc[:train_end])
    features = np.empty((n, len(encoder.feature_names)), dtype=np.float32)
...
def prepare_graph(path, max_rows, ratios):
    return build_graph(load_transactions(path, max_rows), ratios)
...
class FeatureStore:
    ...
    def get(self, ids, target_device):
        ...
        result[is_transaction, :self.transaction.shape[1]] = self.transaction[ids[is_transaction]]
