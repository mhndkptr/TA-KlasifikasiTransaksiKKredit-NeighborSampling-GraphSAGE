...
from .graph import FeatureStore
...
def run_one(cfg, graph, strategy, seed, device):
    ...
    run_name = f"{cfg['experiment']['name']}_{group}_{strategy}_seed{seed}"
    output = Path(cfg["paths"]["result_dir"]) / run_name
    checkpoint = Path(cfg["paths"]["model_dir"]) / run_name / "best.pt"
    if (output.exists() or checkpoint.exists()) and not cfg["experiment"]["overwrite"]:
        raise FileExistsError(f"Run sudah ada: {output}. Gunakan nama baru atau --overwrite secara eksplisit.")
    output.mkdir(parents=True, exist_ok=True)
    # A failed overwrite must never leave a stale completed metrics file.
    ...
    best_state, train_table = None, None
    history = []
    ...
    for epoch in progress(range(1, tc["epochs"]+1), enabled, desc=f"{strategy} seed {seed}", unit="epoch"):
        ...
        seen = 0
        batches = training_batches(total, tc["batch_size"], generator)
        for roots in progress(batches, enabled, total=math.ceil(total/tc["batch_size"]), desc=f"train {epoch}", leave=False):
            roots = roots.to(device)
            blocks = sample_blocks(graph, roots, train_table, sc["fanouts"])
            features = store.get(blocks[0].source, device)
            optimizer.zero_grad(set_to_none=True)
            logits = model(features, blocks)
            loss = F.binary_cross_entropy_with_logits(logits, labels[roots], pos_weight=pos_weight)
            if not torch.isfinite(loss):
                raise FloatingPointError(f"Loss non-finite pada epoch {epoch}")
            loss.backward()
            if tc["gradient_clip_norm"] > 0:
                torch.nn.utils.clip_grad_norm_(model.parameters(), tc["gradient_clip_norm"], error_if_nonfinite=True)
            optimizer.step()
            loss_sum += loss.detach()*len(roots)
            seen += len(roots)
        contexts, context_seconds = build_contexts(model, graph, store, eval_tables, ec["batch_size"], device)
        ...
        LOGGER.info("%s seed=%d epoch=%d loss=%.5f val_AP=%.5f best=%.5f@%d stale=%d", strategy, seed, epoch, record["loss"], score, best, best_epoch, stale)
        del contexts
    ...
    contexts, final_context_seconds = build_contexts(model, graph, store, eval_tables, ec["batch_size"], device)
    ...
    metrics.update({"strategy": strategy, "seed": seed, "best_epoch": best_epoch, "epochs": len(history),
        ...
        "inference_with_preparation_ms_per_1000": (weight_seconds+eval_table_seconds+final_context_seconds+test_seconds)*1e6/len(test_nodes),
