...
RUNTIME_DEFAULTS = {
    ...
    'evaluation': {'primary_threshold': 'validation', 'calibration_tail_fraction': 1.0},
}
...
def validate_config(cfg):
    if cfg['features']['encoder'] not in {'behavioral', 'robust', 'legacy'} or cfg['features']['storage_dtype'] not in {'float16', 'float32'}:
        raise ValueError('features.encoder behavioral/robust/legacy; storage_dtype float16/float32')
    ...
    if tc['selection_metric'] == 'recent_ap' and tc['selection_fraction'] + cfg['evaluation']['calibration_tail_fraction'] > 1+1e-9:
        raise ValueError('Selection dan calibration EXP11 tidak boleh overlap')
    ...
    if mc["num_layers"] != 2 or len(sc["fanouts"]) != 2 or any(not isinstance(k, int) or k < 2 for k in sc["fanouts"]):
        raise ValueError("EXP11 khusus dua lapisan dengan setiap fanout >= 2")
    if sc["ppr_steps"] != 3:
        raise ValueError("Closed-form PPR EXP11 memerlukan ppr_steps=3")
    ...
    for section, names in {
        ...
        "training": ["batch_size", "epochs", "early_stopping_patience", "min_epochs", "selection_min_fraud"],
        "evaluation": ["batch_size", "sampling_passes", "latency_nodes", "latency_repeats"],
        "runtime": ["cpu_threads"],
    }.items():
        for name in names:
