...
from .protocol import selection_bounds, positive_channel_weights, stopping_decision, validate_selection_labels
...
def _train(cfg, graph, strategy, seed, device, group, attempt):
    ...
    selection_start, selection_end = selection_bounds(len(val_labels), tc, val_labels)
    selection_labels = val_labels[selection_start:selection_end]
    validate_selection_labels(selection_labels)
    calibration_start = int(len(val_labels)*(1-ec['calibration_tail_fraction']))
    if ec['calibrate_threshold'] and len(np.unique(val_labels[calibration_start:])) != 2:
        raise ValueError('Calibration window memerlukan dua kelas; perbesar subset/window tanpa melihat test')
    ...
    LOGGER.info('Selection val[%d:%d]: %d fraud; calibration val[%d:%d]: %d fraud; minimum epochs=%d; patience=%d',
        selection_start, selection_end, int(selection_labels.sum()), calibration_start, len(val_labels),
        int(val_labels[calibration_start:].sum()), tc['min_epochs'], tc['early_stopping_patience'])
    for epoch in progress(range(1, tc["epochs"]+1), enabled, desc=f"{strategy} seed {seed}", unit="epoch"):
        ...
        score, val_blocks = validation_score(selection_labels, val_p[selection_start:selection_end],
        ...
        LOGGER.info("%s seed=%d epoch=%d loss=%.5f val_AP=%.5f best=%.5f@%d stale=%d train=%.1fs (%.0f txn/s) sample=%.1fs means=%.1fs val=%.1fs",
            strategy, seed, epoch, record["loss"], val_ap, best_ap, best_epoch, stale,
    ...
    if ec["calibrate_threshold"]:
        calibration_start = int(len(val_labels)*(1-ec['calibration_tail_fraction']))
        calibrated_threshold, calibration = calibrate_threshold(val_labels[calibration_start:], val_p[calibration_start:], ec['threshold_beta'])
        calibration.update({'scope': 'chronological validation tail', 'start_offset': calibration_start,
            'count': len(val_labels)-calibration_start, 'fraud': int(val_labels[calibration_start:].sum()),
            'tail_fraction': ec['calibration_tail_fraction']})
    ...
    checkpoint_payload.update({'selection_window': {'start_offset': selection_start, 'end_offset': selection_end},
    ...
    result = {"comparison_id": group, "source_manifest": source_manifest(), "config": cfg, "metrics": metrics, "environment": environment(device),
        ...
        "training": {"positive_class_weight": class_weight, "imbalance_ratio": imbalance, "precision": "float32",
            ...
            'selection_window': {'start_offset': selection_start, 'end_offset': selection_end,
                'fraud': int(selection_labels.sum()), 'count': len(selection_labels)},
